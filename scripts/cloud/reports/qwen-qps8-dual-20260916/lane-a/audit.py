...
root=base/'sfs_cloud_results_20260916/sfs-vast/controls/qps8-lane-a'
...
assert d['config']['request_rate_qps']==8.0
...
out={'status':'PASS_INDEPENDENT_CONTROLLER_AUDIT','raw_point':str(p),'raw_sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'requests':16000,'succeeded':16000,'failed':0,'scored_queries':len(common),'qps':8.0,'ttft_slo_attainment_pct':100*statistics.mean(x['system_entry_e2e_ttft_slo_met'] for x in rows),'ontimeutility':{k:statistics.mean(v) for k,v in utility.items()},'arrival_quarters':[]}
