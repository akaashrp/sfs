"""Isolated subset/full Qwen pool sharing exclusive GPU locks with base runs."""
import contextlib
import json
import os
...
from scripts.cloud.fcfs.config import instances,server_argv
...
def pool(bundle, models, output, gpus, indices=(0,1,2), coefficients=None):
    ...
    with locks(Path('/dev/shm')/f'sfs-cloud-locks-{os.getuid()}', ['gpu-'+r[1] for r in fingerprint['gpus']]):
        ...
        try:
            ...
            cfg=instances(bundle,[s.getsockname()[1] for s in socks],tag,coefficients)
            ...
            for index,row in zip(indices,cfg['instances']):
                argv=server_argv(bundle,models[row['model_id']],row,index,output)
            ...
            for row in cfg['instances']:
                while True:
                    if any(p.poll() is not None for p in processes):raise RuntimeError('FCFS server exited; inspect its retained log')
                    try:
                    ...
                    if time.monotonic()>deadline:raise TimeoutError('Bounded FCFS startup exceeded ten minutes')
                    time.sleep(2)
